...
#define UUID_FILE_PATH  "/generated_uuid/.uuid"
...
static bool utils_get_uuid(char *id, uint8_t len, const char *interface)
{
        (void)(interface);
        FILE *stream;

        stream = fopen(UUID_FILE_PATH, "r");
        if (!stream) {
                printf("%s:%d: fopen() failed\n", __func__, __LINE__);
                return false;
        }

        if (!fgets(id, len, stream)) {
                printf("%s:%d: fgets() failed\n", __func__, __LINE__);
                fclose(stream);
                return false;
        }
        size_t flen = strlen(id);
        id[flen - 1] = '\0';
        fclose(stream);
        return true;
}
...
#else
        return utils_get_uuid(id, len, interface);
#endif
}
