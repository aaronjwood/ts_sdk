...
bool utils_get_ip_addr(char *ipaddr, uint8_t length, const char *interface)
{
	...
	if (at_modem_get_ip(ip)) {
		...
		return true;
		 ...
		 dbg_printf("L476: %s:%d: at_modem_get_ip is failed\n",
				__func__, __LINE__);
		return false;
	}
}
...
bool utils_get_device_id(char *id, uint8_t len, char *interface)
{
	...
	if ((at_modem_get_imei(d_id))) {
		snprintf(id, len, "%s", &d_id[2]);
		return true;
	} else {
		dbg_printf("L476: %s:%d: at_modem_get_imei is failed\n",
					__func__, __LINE__);
		return false;
	}
}
...
bool utils_get_manufacturer(char *value, uint32_t len)
{
	...
	if ((at_modem_get_man_info(minfo))) {
		...
		return true;
		 ...
		 dbg_printf("L476: %s:%d: at_modem_get_man_info is failed\n",
				__func__, __LINE__);
		return false;
	}
}
...
bool utils_get_dev_model(char *value, uint32_t len)
{
	...
	if ((at_modem_get_mod_info(model))) {
		...
		dbg_printf("L476: %s:%d: at_modem_get_mod_info is failed\n",
					__func__, __LINE__);
		return false;
	}
}
...
bool utils_get_chipset(char *value, uint32_t len)
{
	if (!value || (len == 0))
		return false;
	snprintf(value, len, "%s", "STM32L476RGT");
	return true;
}
...
bool utils_get_iccid(char *value, uint32_t len)
{
	...
	if ((at_modem_get_iccid(iccid))) {
		...
		return true;
		 ...
		 dbg_printf("L476: %s:%d: at_modem_get_iccid is failed\n",
					__func__, __LINE__);
		return false;
	}
}
...
bool utils_get_local_time(char *value, uint32_t len)
{
	...
	if ((at_modem_get_ttz(time))) {
		...
		dbg_printf("L476: %s:%d: at_modem_get_ttz is failed\n",
				__func__, __LINE__);
		return false;
	}
}
...
bool utils_get_sis(char *value, uint32_t len)
{
	...
	if ((at_modem_get_ss(sigstr))) {
		...
		dbg_printf("L476: %s:%d: at_modem_get_sis is failed\n",
					__func__, __LINE__);
		return false;
	}
}
...
bool utils_get_imsi(char *value, uint32_t len)
{
	...
	if ((at_modem_get_imsi(imsi))) {
		...
		dbg_printf("L476: %s:%d: at_modem_get_imsi is failed\n",
				__func__, __LINE__);
		return false;
	}
}
...
bool utils_get_fwbid(char *value, uint32_t len)
{
	...
	if ((at_modem_get_fwver(bid))) {
		...
		dbg_printf("L476: %s:%d: at_modem_get_fwver is failed\n",
				__func__, __LINE__);
		return false;
	}
}
