...
int main(int argc, char *argv[])
{
	...
	while (!mbedtls_net_init(&ctx)) {
		HAL_Delay(8000);
	}
	dbg_printf("INIT Success\n");
	char temp[18] = "GET \\ HTTP\\1.1\r\n";

	if (mbedtls_net_connect(&ctx, "73.47.119.157", "749",
		MBEDTLS_NET_PROTO_TCP) != 0) {
		dbg_printf("mbedtls_net_connect failed\n");
		while (1) {
		}
	}

	dbg_printf("Sending some data:\n");
	int res = mbedtls_net_send(&ctx, (uint8_t *)temp, 18);
	...
	int r_b;
	while (1) {
		r_b = mbedtls_net_recv(&ctx, buf, 1024);
