...
/*
 * Sends a diagnostic message to the cloud service. This call is blocking in nature.
 * Function publishes message to unit on board topic.
 *
 * Parameters:
 *	buf    : Message to send.
 ...
 * 	PROTO_TIMEOUT   : Timed out sending the message. Sending failed.
 */
proto_result mqtt_send_diag_msg_to_cloud(const void *buf, uint32_t sz, proto_callback cb);
