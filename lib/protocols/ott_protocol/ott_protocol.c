...
#define INVOKE_SEND_CALLBACK(_buf, _evt) \
	do { \
		if (session.send_cb) \
			session.send_cb((_buf), (_evt)); \
	} while(0)

#define INVOKE_RECV_CALLBACK(_buf, _evt) \
	do { \
		if (session.rcv_cb) \
			session.rcv_cb((_buf), (_evt)); \
	} while(0)
...
{
        if (!rcv_buf || (sz >= PROTO_MAX_RCV_MSG_SZ))
                return PROTO_INV_PARAM;
...
retry_connection:
	...
	if (ott_send_auth_to_cloud(c_flags, auth.dev_ID,
				auth.d_sec->sz, auth.d_sec->bytes) != PROTO_OK) {
...
static bool process_recvd_msg(msg_t *msg_ptr, bool invoke_send_cb)
{
	...
	if (OTT_FLAG_IS_SET(c_flags, CF_ACK)) {
		proto_event evt = PROTO_STS_NONE;
		...
		if (m_type == MT_UPDATE) {
			...
			INVOKE_RECV_CALLBACK(&polling_int_ms, PROTO_RCVD_CMD_PI);
			session.pend_ack = true;
		}

		/* Call the callbacks with the type of message received */
		if (m_type == MT_UPDATE || m_type == MT_CMD_SL)
			INVOKE_RECV_CALLBACK(session.rcv_buf, evt);
		if (invoke_send_cb)
			INVOKE_SEND_CALLBACK(NULL, PROTO_RCVD_ACK);
	} else if (OTT_FLAG_IS_SET(c_flags, CF_NACK)) {
		no_nack_detected = false;
		if (invoke_send_cb)
			INVOKE_SEND_CALLBACK(NULL, PROTO_RCVD_NACK);
	}
...
static bool recv_resp_within_timeout(uint32_t timeout, bool invoke_send_cb)
{
	...
	if (end - start >= timeout) {
		if (invoke_send_cb)
			INVOKE_SEND_CALLBACK(NULL, PROTO_SEND_TIMEOUT);
		return false;
	}
