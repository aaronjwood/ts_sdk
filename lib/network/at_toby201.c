...
#define DEBUG_V3(...)	if (debug_level >= 3) printf(__VA_ARGS__)
...
static uint8_t __at_generic_comm_rsp_util(at_command_desc *desc, bool skip_comm,
        ...
        if (uart_line_avail(rsp_header, rsp_trailer) > 0) {
                state |= PROC_URC;
                __at_process_urc();
                state &= ~PROC_URC;
        }
